    ...
    if (conf.handler != HandlerType.None) {
      ...
      conf.handler match {
        ...
        case HandlerType.File => {
          handler = new FileHandler(s"${conf.fileHandler.folder}/${conf.fileHandler.pattern}", conf.fileHandler.append)
        }
        case HandlerType.Socket => {
          handler = new SocketHandler(conf.socketHandler.host, conf.socketHandler.port)
        }
        case HandlerType.Custom =>
          handler = Class.forName(conf.custom.className).newInstance().asInstanceOf[java.util.logging.Handler]
