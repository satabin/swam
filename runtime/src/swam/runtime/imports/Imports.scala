...
object AsInterface {
  ...
  implicit def function0AsInsterface[Ret, F[_]](implicit F: MonadError[F, Throwable],
                                                writer: FunctionReturnWriter[F, Ret]): AsInterface[() => F[Ret], F] =
    new AsInterface[() => F[Ret], F] {
      def view(f: () => F[Ret]) = new IFunction0(f)
    }

  implicit def function1AsInterface[P1, Ret, F[_]](
      implicit F: MonadError[F, Throwable],
      reader1: ValueReader[F, P1],
      writer: FunctionReturnWriter[F, Ret]): AsInterface[(P1) => F[Ret], F] =
  ...
  implicit def function2AsInterface[P1, P2, Ret, F[_]](
      ...
      writer: FunctionReturnWriter[F, Ret]): AsInterface[(P1, P2) => F[Ret], F] =
