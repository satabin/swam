...
class Transformer[F[_], Ctx](implicit F: Monad[F]) {
  ...
  protected[this] val unopTransformId: (Ctx, Unop) => F[(Ctx, Inst)] = {
    case (ctx, u @ i32.Clz)     => i32ClzTransform(ctx, u)
    case (ctx, u @ i32.Ctz)     => i32CtzTransform(ctx, u)
    case (ctx, u @ i32.Popcnt)  => i32PopcntTransform(ctx, u)
    case (ctx, u @ i64.Clz)     => i64ClzTransform(ctx, u)
    case (ctx, u @ i64.Ctz)     => i64CtzTransform(ctx, u)
    case (ctx, u @ i64.Popcnt)  => i64PopcntTransform(ctx, u)
    case (ctx, u @ f32.Abs)     => f32AbsTransform(ctx, u)
    case (ctx, u @ f32.Neg)     => f32NegTransform(ctx, u)
    case (ctx, u @ f32.Sqrt)    => f32SqrtTransform(ctx, u)
    case (ctx, u @ f32.Ceil)    => f32CeilTransform(ctx, u)
    case (ctx, u @ f32.Floor)   => f32FloorTransform(ctx, u)
    case (ctx, u @ f32.Trunc)   => f32TruncTransform(ctx, u)
    case (ctx, u @ f32.Nearest) => f32NearestTransform(ctx, u)
    case (ctx, u @ f64.Abs)     => f64AbsTransform(ctx, u)
    case (ctx, u @ f64.Neg)     => f64NegTransform(ctx, u)
    case (ctx, u @ f64.Sqrt)    => f64SqrtTransform(ctx, u)
    case (ctx, u @ f64.Ceil)    => f64CeilTransform(ctx, u)
    case (ctx, u @ f64.Floor)   => f64FloorTransform(ctx, u)
    case (ctx, u @ f64.Trunc)   => f64TruncTransform(ctx, u)
    case (ctx, u @ f64.Nearest) => f64NearestTransform(ctx, u)
  }
  val unopTransform: (Ctx, Unop) => F[(Ctx, Inst)] = unopTransformId
  protected[this] val binopTransformId: (Ctx, Binop) => F[(Ctx, Inst)] = {
    case (ctx, b @ i32.Add)      => i32AddTransform(ctx, b)
    case (ctx, b @ i32.Sub)      => i32SubTransform(ctx, b)
    case (ctx, b @ i32.Mul)      => i32MulTransform(ctx, b)
    case (ctx, b @ i32.DivS)     => i32DivSTransform(ctx, b)
    case (ctx, b @ i32.DivU)     => i32DivUTransform(ctx, b)
    case (ctx, b @ i32.RemS)     => i32RemSTransform(ctx, b)
    case (ctx, b @ i32.RemU)     => i32RemUTransform(ctx, b)
    case (ctx, b @ i32.And)      => i32AndTransform(ctx, b)
    case (ctx, b @ i32.Or)       => i32OrTransform(ctx, b)
    case (ctx, b @ i32.Xor)      => i32XorTransform(ctx, b)
    case (ctx, b @ i32.Shl)      => i32ShlTransform(ctx, b)
    case (ctx, b @ i32.ShrS)     => i32ShrSTransform(ctx, b)
    case (ctx, b @ i32.ShrU)     => i32ShrUTransform(ctx, b)
    case (ctx, b @ i32.Rotl)     => i32RotlTransform(ctx, b)
    case (ctx, b @ i32.Rotr)     => i32RotrTransform(ctx, b)
    case (ctx, b @ i64.Add)      => i64AddTransform(ctx, b)
    case (ctx, b @ i64.Sub)      => i64SubTransform(ctx, b)
    case (ctx, b @ i64.Mul)      => i64MulTransform(ctx, b)
    case (ctx, b @ i64.DivS)     => i64DivSTransform(ctx, b)
    case (ctx, b @ i64.DivU)     => i64DivUTransform(ctx, b)
    case (ctx, b @ i64.RemS)     => i64RemSTransform(ctx, b)
    case (ctx, b @ i64.RemU)     => i64RemUTransform(ctx, b)
    case (ctx, b @ i64.And)      => i64AndTransform(ctx, b)
    case (ctx, b @ i64.Or)       => i64OrTransform(ctx, b)
    case (ctx, b @ i64.Xor)      => i64XorTransform(ctx, b)
    case (ctx, b @ i64.Shl)      => i64ShlTransform(ctx, b)
    case (ctx, b @ i64.ShrS)     => i64ShrSTransform(ctx, b)
    case (ctx, b @ i64.ShrU)     => i64ShrUTransform(ctx, b)
    case (ctx, b @ i64.Rotl)     => i64RotlTransform(ctx, b)
    case (ctx, b @ i64.Rotr)     => i64RotrTransform(ctx, b)
    case (ctx, b @ f32.Add)      => f32AddTransform(ctx, b)
    case (ctx, b @ f32.Sub)      => f32SubTransform(ctx, b)
    case (ctx, b @ f32.Mul)      => f32MulTransform(ctx, b)
    case (ctx, b @ f32.Div)      => f32DivTransform(ctx, b)
    case (ctx, b @ f32.Min)      => f32MinTransform(ctx, b)
    case (ctx, b @ f32.Max)      => f32MaxTransform(ctx, b)
    case (ctx, b @ f32.Copysign) => f32CopysignTransform(ctx, b)
    case (ctx, b @ f64.Add)      => f64AddTransform(ctx, b)
    case (ctx, b @ f64.Sub)      => f64SubTransform(ctx, b)
    case (ctx, b @ f64.Mul)      => f64MulTransform(ctx, b)
    case (ctx, b @ f64.Div)      => f64DivTransform(ctx, b)
    case (ctx, b @ f64.Min)      => f64MinTransform(ctx, b)
    case (ctx, b @ f64.Max)      => f64MaxTransform(ctx, b)
    case (ctx, b @ f64.Copysign) => f64CopysignTransform(ctx, b)
  }
  val binopTransform: (Ctx, Binop) => F[(Ctx, Inst)] = binopTransformId
  protected[this] val testopTransformId: (Ctx, Testop) => F[(Ctx, Inst)] = {
    case (ctx, t @ i32.Eqz) => i32EqzTransform(ctx, t)
    case (ctx, t @ i64.Eqz) => i64EqzTransform(ctx, t)
  }
  ...
  protected[this] val convertopTransformId: (Ctx, Convertop) => F[(Ctx, Inst)] = {
    case (ctx, c @ i32.WrapI64)        => i32WrapI64Transform(ctx, c)
    case (ctx, c @ i32.TruncSF32)      => i32TruncSF32Transform(ctx, c)
    case (ctx, c @ i32.TruncUF32)      => i32TruncUF32Transform(ctx, c)
    case (ctx, c @ i32.TruncSF64)      => i32TruncSF64Transform(ctx, c)
    case (ctx, c @ i32.TruncUF64)      => i32TruncUF64Transform(ctx, c)
    case (ctx, c @ i32.ReinterpretF32) => i32ReinterpretF32Transform(ctx, c)
    case (ctx, c @ i64.ExtendSI32)     => i64ExtendSI32Transform(ctx, c)
    case (ctx, c @ i64.ExtendUI32)     => i64ExtendUI32Transform(ctx, c)
    case (ctx, c @ i64.TruncSF32)      => i64TruncSF32Transform(ctx, c)
    case (ctx, c @ i64.TruncUF32)      => i64TruncUF32Transform(ctx, c)
    case (ctx, c @ i64.TruncSF64)      => i64TruncSF64Transform(ctx, c)
    case (ctx, c @ i64.TruncUF64)      => i64TruncUF64Transform(ctx, c)
    case (ctx, c @ i64.ReinterpretF64) => i64ReinterpretF64Transform(ctx, c)
    case (ctx, c @ f32.DemoteF64)      => f32DemoteF64Transform(ctx, c)
    case (ctx, c @ f32.ConvertSI32)    => f32ConvertSI32Transform(ctx, c)
    case (ctx, c @ f32.ConvertUI32)    => f32ConvertUI32Transform(ctx, c)
    case (ctx, c @ f32.ConvertSI64)    => f32ConvertSI64Transform(ctx, c)
    case (ctx, c @ f32.ConvertUI64)    => f32ConvertUI64Transform(ctx, c)
    case (ctx, c @ f32.ReinterpretI32) => f32ReinterpretI32Transform(ctx, c)
    case (ctx, c @ f64.PromoteF32)     => f64PromoteF32Transform(ctx, c)
    case (ctx, c @ f64.ConvertSI32)    => f64ConvertSI32Transform(ctx, c)
    case (ctx, c @ f64.ConvertUI32)    => f64ConvertUI32Transform(ctx, c)
    case (ctx, c @ f64.ConvertSI64)    => f64ConvertSI64Transform(ctx, c)
    case (ctx, c @ f64.ConvertUI64)    => f64ConvertUI64Transform(ctx, c)
    case (ctx, c @ f64.ReinterpretI64) => f64ReinterpretI64Transform(ctx, c)
  }
  ...
  protected[this] val relopTransformId: (Ctx, Relop) => F[(Ctx, Inst)] = {
    case (ctx, r @ i32.Eq)  => i32EqTransform(ctx, r)
    case (ctx, r @ i32.Ne)  => i32NeTransform(ctx, r)
    case (ctx, r @ i32.LtS) => i32LtSTransform(ctx, r)
    case (ctx, r @ i32.LtU) => i32LtUTransform(ctx, r)
    case (ctx, r @ i32.GtS) => i32GtSTransform(ctx, r)
    case (ctx, r @ i32.GtU) => i32GtUTransform(ctx, r)
    case (ctx, r @ i32.LeS) => i32LeSTransform(ctx, r)
    case (ctx, r @ i32.LeU) => i32LeUTransform(ctx, r)
    case (ctx, r @ i32.GeS) => i32GeSTransform(ctx, r)
    case (ctx, r @ i32.GeU) => i32GeUTransform(ctx, r)
    case (ctx, r @ i64.Eq)  => i64EqTransform(ctx, r)
    case (ctx, r @ i64.Ne)  => i64NeTransform(ctx, r)
    case (ctx, r @ i64.LtS) => i64LtSTransform(ctx, r)
    case (ctx, r @ i64.LtU) => i64LtUTransform(ctx, r)
    case (ctx, r @ i64.GtS) => i64GtSTransform(ctx, r)
    case (ctx, r @ i64.GtU) => i64GtUTransform(ctx, r)
    case (ctx, r @ i64.LeS) => i64LeSTransform(ctx, r)
    case (ctx, r @ i64.LeU) => i64LeUTransform(ctx, r)
    case (ctx, r @ i64.GeS) => i64GeSTransform(ctx, r)
    case (ctx, r @ i64.GeU) => i64GeUTransform(ctx, r)
    case (ctx, r @ f32.Eq)  => f32EqTransform(ctx, r)
    case (ctx, r @ f32.Ne)  => f32NeTransform(ctx, r)
    case (ctx, r @ f32.Lt)  => f32LtTransform(ctx, r)
    case (ctx, r @ f32.Gt)  => f32GtTransform(ctx, r)
    case (ctx, r @ f32.Le)  => f32LeTransform(ctx, r)
    case (ctx, r @ f32.Ge)  => f32GeTransform(ctx, r)
    case (ctx, r @ f64.Eq)  => f64EqTransform(ctx, r)
    case (ctx, r @ f64.Ne)  => f64NeTransform(ctx, r)
    case (ctx, r @ f64.Lt)  => f64LtTransform(ctx, r)
    case (ctx, r @ f64.Gt)  => f64GtTransform(ctx, r)
    case (ctx, r @ f64.Le)  => f64LeTransform(ctx, r)
    case (ctx, r @ f64.Ge)  => f64GeTransform(ctx, r)
  }
  ...
  final def transform(ctx: Ctx, inst: Inst): F[(Ctx, Inst)] = inst match {
    ...
    case inst @ Drop             => dropTransform(ctx, inst)
    case inst @ Select           => selectTransform(ctx, inst)
    case inst @ VarInst(_)       => varInstTransform(ctx, inst)
    case inst @ MemorySize       => memorySizeTransform(ctx, inst)
    case inst @ MemoryGrow       => memoryGrowTransform(ctx, inst)
    case inst @ Nop              => nopTransform(ctx, inst)
    case inst @ Unreachable      => unreachableTransform(ctx, inst)
    ...
    case inst @ Return           => returnTransform(ctx, inst)
