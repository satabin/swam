...
class Resolver[F[_]](implicit F: MonadError[F, Throwable]) {
  ...
  def resolve(instr: Inst, ctx: ResolverContext): F[(r.Inst, ResolverContext)] =
    instr match {
      ...
      case GetLocal(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.GetLocal(idx), ctx))
      case SetLocal(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.SetLocal(idx), ctx))
      case TeeLocal(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.TeeLocal(idx), ctx))
      case GetGlobal(idx) =>
        resolveIndex(idx, instr.pos, ctx.globals, "global").map(idx => (r.GetGlobal(idx), ctx))
      case SetGlobal(idx) =>
        resolveIndex(idx, instr.pos, ctx.globals, "global").map(idx => (r.SetGlobal(idx), ctx))
