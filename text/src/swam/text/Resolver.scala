...
class Resolver[F[_]](implicit F: MonadError[F, Throwable]) {
  ...
  def resolve(instr: Inst, ctx: ResolverContext): F[(r.Inst, ResolverContext)] =
    instr match {
      ...
      case Block(label, tpe, is, endlabel) => resolveBlock(label, tpe, is, endlabel, ctx, instr.pos)
      case Loop(label, tpe, is, endlabel)  => resolveLoop(label, tpe, is, endlabel, ctx, instr.pos)
