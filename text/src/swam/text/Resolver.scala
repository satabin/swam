...
class Resolver[F[_]](implicit F: MonadError[F, Throwable]) {
  ...
  def resolve(instr: Inst, ctx: ResolverContext): F[(r.Inst, ResolverContext)] =
    instr match {
      ...
      case LocalGet(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.LocalGet(idx), ctx))
      case LocalSet(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.LocalSet(idx), ctx))
      case LocalTee(idx) =>
        resolveIndex(idx, instr.pos, ctx.locals, "local").map(idx => (r.LocalTee(idx), ctx))
      case GlobalGet(idx) =>
        resolveIndex(idx, instr.pos, ctx.globals, "global").map(idx => (r.GlobalGet(idx), ctx))
      case GlobalSet(idx) =>
        resolveIndex(idx, instr.pos, ctx.globals, "global").map(idx => (r.GlobalSet(idx), ctx))
      ...
      case Block(label, tpe, is, endlabel) =>
        resolveBlock(label, tpe, is, endlabel, ctx, instr.pos)
      case Loop(label, tpe, is, endlabel) =>
        resolveLoop(label, tpe, is, endlabel, ctx, instr.pos)
      case If(label, tpe, theni, elselabel, elsei, endlabel) =>
        resolveIf(label, tpe, theni, elselabel, elsei, endlabel, ctx, instr.pos)
      case Br(label) =>
        resolveIndex(label, instr.pos, ctx.labels, "label").map(idx => (r.Br(idx), ctx))
      case BrIf(label) =>
        resolveIndex(label, instr.pos, ctx.labels, "label").map(idx => (r.BrIf(idx), ctx))
      ...
      case CallIndirect(TypeUse(tpe, params, results)) =>
